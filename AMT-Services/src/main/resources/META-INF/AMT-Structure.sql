...
/*  Created On : 18-Nov-2017 7:24:50 PM 				*/
/*  DBMS       : MySql 						*/
/* ---------------------------------------------------- */
...
(
	...
	`chapter_status` VARCHAR(2) NOT NULL,
	`start_date` DATE 	,
	`due_date` DATE 	,
	CONSTRAINT `PK_chapter` PRIMARY KEY (`chapter_id` ASC)
)
...
ALTER TABLE `chapter` 
 ADD CONSTRAINT `FK_chapter_content_status`
	FOREIGN KEY (`chapter_status`) REFERENCES `content_status` (`status`) ON DELETE Restrict ON UPDATE Restrict
;
