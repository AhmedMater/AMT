...
/*  Created On : 09-Feb-2018 11:28:32 AM 				*/
/*  DBMS       : MySql 						*/
/* ---------------------------------------------------- */
...
(
	...
	CONSTRAINT `PK_am_app_users` PRIMARY KEY (`app_id` ASC)
)
...
(
	...
	CONSTRAINT `PK_am_users` PRIMARY KEY (`username` ASC)
)
...
(
	`user_id` INT NOT NULL,
	`role` VARCHAR(2) NOT NULL,
	CONSTRAINT `PK_user_role` PRIMARY KEY (`user_id` ASC, `role` ASC)
)
...
ALTER TABLE `users` 
 ADD CONSTRAINT `UNIQUE_Username` UNIQUE (`user_name` ASC)
;
